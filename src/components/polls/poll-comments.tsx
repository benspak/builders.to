...
import { User, Send, Loader2, MoreHorizontal, Trash2, X, MessageCircle } from "lucide-react";
...
export function PollComments({
  ...
  async function handleDelete(commentId: string) {
    ...
    try {
      const response = await fetch(`/api/poll-comments/${commentId}`, {
        method: "DELETE",
      });
  ...
  const modalContent = isOpen && mounted ? (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gradient-to-r from-violet-500/10 via-fuchsia-500/10 to-pink-500/10">
          ...
          <div className="p-6 space-y-4">
            ...
            {!isLoading && comments.length === 0 && (
              <div className="text-center py-8">
                <MessageCircle className="h-12 w-12 mx-auto mb-3 text-violet-400 opacity-30" />
        ...
        <div className="border-t border-white/10 p-4 bg-zinc-900/95">
          ...
          <form onSubmit={handleSubmit} className="flex gap-3">
            ...
            <button
              ...
              className="rounded-xl px-4 py-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0 bg-violet-500/20 text-violet-400 hover:bg-violet-500/30"
            >
              {isSubmitting ? (
  ...
  return (
    <>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-xs text-zinc-500 transition-all hover:text-violet-400 hover:bg-violet-500/10"
      >
