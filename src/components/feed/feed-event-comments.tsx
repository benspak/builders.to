...
interface FeedEventCommentsProps {
  feedEventId: string;
...
export function FeedEventComments({
  ...
  async function handleDelete(commentId: string) {
    ...
    try {
      const response = await fetch(`/api/feed-event-comments/${commentId}`, {
        method: "DELETE",
      });
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={cn("rounded-xl px-4 py-2 text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed", colors.submit)}
              >
...
function CommentItem({ comment, currentUserId, onDelete, onEdit, accentColor = "orange", onVote }: CommentItemProps) {
  const router = useRouter();
